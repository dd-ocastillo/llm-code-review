...
def get_review(
    ...
    for chunked_diff in chunked_diff_list:
        ...
        prompt = template.format(question=chunked_diff)
        review_result = llm.complete(prompt)
        chunked_reviews.append(str(review_result))
    ...
    summarized_review = llm.complete(prompt)
